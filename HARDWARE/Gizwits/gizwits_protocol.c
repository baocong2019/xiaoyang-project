...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagSwitch)
    {
        ...
        info->num++;
        dataPoints->valueSwitch = gizStandardDecompressionValue(Switch_BYTEOFFSET,Switch_BITOFFSET,Switch_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSwitchtime)
    {
        info->event[info->num] = EVENT_Switchtime;
        info->num++;
        dataPoints->valueSwitchtime = gizStandardDecompressionValue(Switchtime_BYTEOFFSET,Switchtime_BITOFFSET,Switchtime_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagG_temp_net)
    {
        info->event[info->num] = EVENT_G_temp_net;
        info->num++;
        dataPoints->valueG_temp_net = gizX2Y(G_temp_net_RATIO,  G_temp_net_ADDITION, issuedData->attrVals.valueG_temp_net); 
    }
        
    if(0x01 == issuedData->attrFlags.flagfanspeed)
    {
        info->event[info->num] = EVENT_fanspeed;
        info->num++;
        dataPoints->valuefanspeed = gizX2Y(fanspeed_RATIO,  fanspeed_ADDITION, issuedData->attrVals.valuefanspeed); 
    }
        
    if(0x01 == issuedData->attrFlags.flagHour)
    {
        info->event[info->num] = EVENT_Hour;
        info->num++;
        dataPoints->valueHour = gizX2Y(Hour_RATIO,  Hour_ADDITION, issuedData->attrVals.valueHour); 
    }
        
    if(0x01 == issuedData->attrFlags.flagMint)
    {
        info->event[info->num] = EVENT_Mint;
        info->num++;
        dataPoints->valueMint = gizX2Y(Mint_RATIO,  Mint_ADDITION, issuedData->attrVals.valueMint); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueSwitchtime != cur->valueSwitchtime)
    {
        GIZWITS_LOG("valueSwitchtime Changed\n");
        ret = 1;
    }
    if(last->valueG_temp_net != cur->valueG_temp_net)
    {
        GIZWITS_LOG("valueG_temp_net Changed\n");
        ret = 1;
    }
    if(last->valuefanspeed != cur->valuefanspeed)
    {
        GIZWITS_LOG("valuefanspeed Changed\n");
        ret = 1;
    }
    ...
    if(last->valueMint != cur->valueMint)
    {
        GIZWITS_LOG("valueMint Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Switch_BYTEOFFSET,Switch_BITOFFSET,Switch_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSwitch);
    gizStandardCompressValue(Switchtime_BYTEOFFSET,Switchtime_BITOFFSET,Switchtime_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSwitchtime);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueG_temp_net = gizY2X(G_temp_net_RATIO,  G_temp_net_ADDITION, dataPoints->valueG_temp_net); 
    devStatusPtr->valuefanspeed = gizY2X(fanspeed_RATIO,  fanspeed_ADDITION, dataPoints->valuefanspeed); 
    devStatusPtr->valueHour = gizY2X(Hour_RATIO,  Hour_ADDITION, dataPoints->valueHour); 
    devStatusPtr->valueMint = gizY2X(Mint_RATIO,  Mint_ADDITION, dataPoints->valueMint); 
